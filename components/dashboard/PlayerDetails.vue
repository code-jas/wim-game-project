<template>
  <div
    class="w-full h-[274px] bg-white rounded-[15px] drop-shadow-lg py-8 px-7 poppins text-lt-t-prim dark:bg-dk-inp-b-alt dark:text-violet-l md:h-[274px]"
  >
    <h3>Player Name</h3>
    ...
    <div
      class="flex items-center justify-evenly mt-10 gap-10 text-center md:flex-row"
    >
  ...
  methods: {
    ...
    getTotalAccuracy() {
      ...
      for (const difficulty in this.selectedPlayer.accuracy) {
        if (
          Object.hasOwnProperty.call(this.selectedPlayer.accuracy, difficulty)
        ) {
