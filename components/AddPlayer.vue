<template>
  <div class="relative p-20 bg-lt-dash" :class="{ 'dark:bg-dk-inp-b': isDark }">
    ...
    >
      ...
      >
        <i
          class="uil uil-question-circle text-5xl hover:scale-105 transition duration-200 text-blue bg-violet-l-alt"
          :class="{ 'dark:text-dk-t-prim dark:bg-dk-inp-alt': isDark }"
        ></i>
      </div>
    </div>

    <h1
      class="absolute top-20 left-1/2 transform -translate-x-1/2 text-6xl font-franklin text-blue-v"
      :class="{ 'dark:text-dk-t-prim': isDark }"
    >
      Image/Word <br />
      Match Game
    </h1>
    ...
    <div class="flex items-center justify-center min-h-screen flex-col">
      ...
      <button
        type="button"
        @click="addPlayer()"
        class="w-[280px] h-[70px] bg-blue-v font-medium font-franklin text-dk-t-prim-alt text-5xl rounded-[15px] px-5 py-2.5 mr-2 mb-2 hover:bg-blue-v-alt focus:ring-4 focus:ring-blue-300 hover:scale-105 active:transform active:translate-y-2 transition duration-300"
      >
        Start
      </button>
    </div>
